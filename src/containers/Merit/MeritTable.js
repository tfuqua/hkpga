...
const MeritRow = merit => {
  if (merit.count > 0) {
    return <tr>{Object.keys(merit.entry).map((cell, i) => <td key={i}>{merit.entry[cell]}</td>)}</tr>;
  } else {
    return <tr>{Object.keys(merit.entry).map((cell, i) => <th key={i}>{merit.entry[cell]}</th>)}</tr>;
  }
};
